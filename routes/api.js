...
loghandler = {
    notparam: {
        ...
        creator: `${creator}`,
        code: 406,
        message: 'parâmetros de entrada de apikey',
        getApikey: 'Chave API??? Contate-me no WhatsApp'
    },
    notkey: {
        status: false,
        creator: `${creator}`,
        code: 406,
        message: 'parâmetros-chave de entrada'
    },
    noturl: {
        status: false,
        creator: `${creator}`,
    ...
    notnabi: {
        ...
        creator: `${creator}`,
        code: 406,
        message: 'parâmetro de entrada do profeta'
    },
    nottext3: {
        status: false,
        creator: `${creator}`,
        code: 406,
        message: 'parâmetro de entrada texto3'
    },
    nottheme: {
        status: false,
        creator: `${creator}`,
        code: 406,
        message: 'insira os parâmetros do tema'
    },
    notusername: {
        status: false,
        creator: `${creator}`,
        code: 406,
        message: 'insira o parâmetro de nome de usuário'
    },
    notvalue: {
        status: false,
        creator: `${creator}`,
        code: 406,
        message: 'valores de parâmetro de entrada '
    },
    notheme: {
    	status: false,
        creator: `${creator}`,
        code: 406,
        message: 'o tema não está disponível, digite texmaker / list ou leia a documentação '
     },
    invalidKey: {
        status: false,
        creator: `${creator}`,
        code: 406,
        message: 'Chave API??? Contate-me no WhatsApp '
    },
    invalidlink: {
        status: false,
        creator: `${creator}`,
    ...
    notAddApiKey: {
        ...
        creator: `${creator}`,
        code: 406,
        message: 'parâmetro de status de entrada, apikeyInput, e-mail, número de telefone, nome, idade, país, exp '
    },
    error: {
        status: false,
        creator: `${creator}`,
        message: 'pode estar em conserto '
    }
}

const listkey = ["frekey", "venomofc"];
...
router.get('/addapikey', (req, res, next) => {
    ...
    if (apikey != 'Venomofc') return res.json(loghandler.invalidKey)
    ...
    try {
        ...
        .then(() => {
              res.json({
                  status: true,
                  creator: `${creator}`,
                  result: 'dados adicionados com sucesso, status: ' + status + ', apikey : ' + apikeyInput + ', email : ' + email + ', nomor_hp : ' + nomorhp + ', name :  ' + name + ', age : ' + age + ', country : ' + country + ', exp : ' + exp
              })
        })
    } catch (e) {
        console.log(e)
    ...
    try {
        ...
        .then(() => {
             res.json({
                  status: true,
                  creator: `${creator}`,
                  result: 'dados excluídos com sucesso, status : ' + status + ', apikey : ' + apikeyInput + ', email : ' + email + ', nomor_hp : ' + nomorhp + ', name :  ' + name + ', age : ' + age + ', country : ' + country + ', exp : ' + exp
              })
        })
    } catch (e) {
        console.log(e)
...
router.get('/cewe/vietnam', async (req, res, next) => {
        var apikeyInput = req.query.apikey
	if(!apikeyInput) return res.json(loghandler.notparam)	
	if (apikeyInput != 'Venomofc')  return res.json(loghandler.invalidKey)
...
router.get('/infonpm', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'Venomofc') return res.json(loghandler.invalidKey)
    if (!query) return res.json({ status : false, creator : `${creator}`, message : "Insira os parâmetros do nome de  query"})
...
router.get('/nulis', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'Venomofc') return res.json(loghandler.invalidKey)
    if (!text) return res.json({ status : false, creator : `${creator}`, message : "parâmetros de entrada de texto"})
...
router.get('/hadits', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'Venomofc') return res.json(loghandler.invalidKey)
    if (!kitab) return res.json({ status : false, creator : `${creator}`, message : "masukan parameter livro"})
    if (!nomor) return res.json({ status : false, creator : `${creator}`, message : "Insira os parâmetros de  número"})
...
router.get('/quran', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'Venomofc') return res.json(loghandler.invalidKey)
    if (!surah) return res.json({ status : false, creator : `${creator}`, message : "Insira os parâmetros de  surah"})
    if (!ayat) return res.json({ status : false, creator : `${creator}`, message : "Insira os parâmetros de  ayat"})
...
router.get('/fbdown', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'Venomofc') return res.json(loghandler.invalidKey)
    if (!url) return res.json({ status : false, creator : `${creator}`, message : "Insira os parâmetros de  url"})
...
router.get('/wikipedia', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'Venomofc') return res.json(loghandler.invalidKey)
        if(!search) return res.json({ status : false, creator : `${creator}`, message : "Insira os parâmetros de  search"})
...
router.get('/fakedata', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'Venomofc') return res.json(loghandler.invalidKey)
        if(!country) return res.json({ status : false, creator : `${creator}`, message : "Insira os parâmetros de  country"})
...
router.get('/liriklagu', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'Venomofc') return res.json(loghandler.invalidKey)
        if(!lagu) return res.json({ status : false, creator : `${creator}`, message : "Insira os parâmetros de  kata"})
...
router.get('/infocuaca', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'Venomofc') return res.json(loghandler.invalidKey)
	if(!provinsi) return res.json({ status : false, creator : `${creator}`, message : "Insira os parâmetros de  provinsi"})
...
router.get('/news/cnn', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'Venomofc') return res.json(loghandler.invalidKey)
    if (!type) return res.json({ status : false, creator : `${creator}`, message : "Insira os parâmetros de  type"})
...
router.get('/filmapik/search', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'Venomofc') return res.json(loghandler.invalidKey)
    if (!film) return res.json({ status : false, creator : `${creator}`, message : "Insira os parâmetros de  film"})
...
router.get('/filmapik/play', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'Venomofc') return res.json(loghandler.invalidKey)
    if (!id) return res.json({ status : false, creator : `${creator}`, message : "Insira os parâmetros de  id"})
...
router.get('/lk21/year', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'Venomofc') return res.json(loghandler.invalidKey)
    if (!tahun) return res.json({ status : false, creator : `${creator}`, message : "Insira os parâmetros de  tahun"})
...
router.get('/lk21/country', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'Venomofc') return res.json(loghandler.invalidKey)
    if (!negara) return res.json({ status : false, creator : `${creator}`, message : "Insira os parâmetros de  negara"})
...
router.get('/lk21/genre', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'Venomofc') return res.json(loghandler.invalidKey)
    if (!tipe) return res.json({ status : false, creator : `${creator}`, message : "Insira os parâmetros de  tipe"})
...
router.get('/ig/stalk', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'Venomofc') return res.json(loghandler.invalidKey)
    if (!username) return res.json({ status : false, creator : `${creator}`, message : "Insira os parâmetros do nome de usuário"})
...
router.get('/yutub/search', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'Venomofc') return res.json(loghandler.invalidKey)
    if (!video) return res.json({ status : false, creator : `${creator}`, message : "parâmetros de entrada de  video"})
